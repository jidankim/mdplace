...
**Processing Envelope**:
A closed, immutable, least-privilege input for one Intelligence Adapter Attempt that binds the exact approved Processing Policy and Source Profile versions, provider, purpose, destination, transmitted fields and artifacts, applied redactions, effective capabilities, retention facts, credential boundary, contract versions, and input/output/runtime/cost ceilings before any payload byte is transmitted. A remote destination uses only its bound credential-free HTTPS endpoint with a closed DNS-style authority and default port; a local destination uses only its equivalently closed local endpoint and is not network egress. It contains untrusted data, never instructions or authority.
...
**Adapter Run Receipt**:
The deterministic, immutable operational record for one Intelligence Adapter Attempt, binding its Processing Envelope, exact transmission observation, destination, capabilities, retention artifacts, credential boundary, canonical observed timestamps reconciled exactly with measured runtime, provider request identifier when available, resource use, isolation and canary evidence, output artifacts, outcome, and reason. Invalid or calendar-impossible observation timestamps are rejected before an attempt observation and are never replaced with invented facts. Its reason is the earliest applicable condition in the closed global precedence, even when several failures are observed. Recovery names one exact initial, retry, or fallback attempt and accepts its receipt only when a compatible crash boundary and the same closed evaluator recompute the preceding cumulative chain state and complete target receipt. It contains no credential or secret and grants no semantic or filesystem authority.
