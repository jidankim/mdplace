...
**Remote Intelligence Adapter**:
The independently claimable remote Conformance Profile of an Intelligence Adapter. Each permitted attempt is isolated, ephemeral, advisory-only, bound to one exact Processing Envelope and approved HTTPS destination, and evidenced by the exact transmitted bytes; every denial occurs before egress with zero transmitted bytes. Its credential boundary is a prerequisite fact only, disclosed provider facts remain evidence-bound, and unproven provider facts remain unsupported or inconclusive.
_Avoid_: Remote Adapter, remote authority, authenticated provider guarantee

**Codex Intelligence Adapter**:
The independently claimable, specification-only Conformance Profile that applies the Intelligence Adapter protocol to one documented non-interactive Codex boundary. Conformance requires current exact evidence for the interface, opaque authentication prerequisite, effective capabilities, network destination, Processing Envelope, transmitted bytes, bounded output, denial, receipt, and recovery behavior; it does not assert live Codex behavior or grant tool, semantic, filesystem, note-placement, taxonomy, projection, command, or automation authority.
_Avoid_: Codex integration, live Codex guarantee, authenticated Codex authority
