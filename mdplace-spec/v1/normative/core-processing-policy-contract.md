...
Every Processing Policy MUST conform to `contracts/schemas/processing-policy.schema.json`. It MUST name one vault, stable policy identity and exact version, lifecycle state, optional exact parent binding, explicit grants, local redaction obligations, provider-declared retention facts, and a nondelegated approval record. Its approval MUST bind the canonical policy payload and a separately validated, digest-bound vault-owner approval receipt present in the manifest-bound `contracts/processing-policy-trust-store.json` readback rather than in caller-supplied scenario data. That readback MUST select exactly one scenario and vault, exact current policy or Source Profile bytes and lifecycle state, owner principal, and trusted receipt digests; missing, duplicate, stale, revoked, or mismatched bindings deny. It MUST contain no credential value. A credential reference is usable only inside an exact boundary that names the operating-system credential store, authentication method, permitted provider, and permitted purposes.

The grants are exhaustive. They MUST explicitly enumerate Intelligence Adapter and provider identifiers, purposes, field and data-class grants, disclosure class, derived artifact kinds, destinations with a closed local-or-remote classification and exact endpoints, credential boundaries, exact consent bindings, per-attempt input/output/runtime/cost budgets, aggregate retry-chain input/output/elapsed-time/cost budgets, an ordered fallback chain, capabilities, non-authoritative semantic scope, and automation scope. Each consent binding correlates one exact adapter, provider, purpose, authorized field and artifact sets, destination, credential boundary, and retention fact; independently enumerated values never create an implicit cross-product. A `local_only` field never authorizes a `remote` destination, and destination classification MUST agree with its closed endpoint scheme. Retention facts MUST bind data-use terms, region, and subprocessors. The non-configurable v1 ceiling is one safe-transient retry against the same adapter and one pre-authorized fallback adapter; each fallback row binds its adapter and exact consent binding, and every attempt appears in order in the closed attempt chain. Every attempt MUST reference a manifest-bound trusted local accounting receipt that binds a digest of the complete semantic request (all request fields with only the circular receipt identifiers and digests omitted), payload, adapter, consent, position, outcome, and measured or reserved resource usage. Each receipt's usage MUST remain within the exact per-attempt request budget. The aggregate retry counters MUST equal the sums recomputed from those receipts and include initial, retry, and fallback attempts; caller-provided aggregate assertions are not evidence. An empty collection grants nothing. A missing field, unknown field, wildcard, duplicate stable identifier, ambient default, inferred provider capability, or unbound fallback grants nothing.

The issue #8 approval is standing consent, not per-run approval: an unchanged, active, exact policy and its authenticated approval receipt MAY authorize routine runs without another prompt. Reuse is valid only for those exact immutable policy bytes, current lifecycle binding, vault, and consent binding. A changed policy payload, version, lifecycle state, destination, data class, retention term, authentication boundary, or approval receipt requires fresh trusted readback and cannot replay the earlier approval.
...
A processing decision MUST validate its closed request, exact consent-binding identifier, and exact policy binding before inspecting permission. It MUST compare the actual UTF-8 payload byte length with the declared input budget; a caller-provided counter cannot substitute for the measured length. An intake decision MUST additionally validate and read back the active Source Profile before interpreting a Capture Candidate or evaluating any Processing Policy grant. The following precedence is binding; the first failure denies the request, emits a deterministic denial receipt bound to the complete request digest, payload digest, selected consent-binding digest, policy and Source Profile references, and zero network effect, transmits zero bytes, creates no semantic operation, and has no vault filesystem effect.
...
| 9 | A trusted local attempt-accounting receipt is missing, mismatched, or unbound, or a retry attempt, recomputed aggregate input/output, elapsed time, cumulative cost, same-adapter identity, ordered attempt, or safe-transient outcome violates its exact bound | Terminate the chain and record denial |
| 10 | Fallback is absent from the ordered chain or does not match its exact adapter, consent binding, provider, purpose, destination, and credential boundary | Deny; never silently substitute |
| 11 | Capability, semantic scope, or automation scope is not enumerated | Deny without side effect or semantic write |
| 12 | A required trusted, request/policy/payload/field-bound local redaction receipt is missing, or a destination-bound retention/data-use/region/subprocessor fact is missing; unknown retention or training/data-use terms lack risk acknowledgment | Deny and transmit zero bytes |
...
Every fixture MUST declare observable inputs, exact payload bytes, outputs, operations, closed receipts, filesystem and network effects, terminal state, and illegal-transition status. The fixture pack MUST also carry one exact oracle for every denied row in both lifecycle tables and exact public observations for missing, torn, mismatched, and ambiguous recovery evidence without increasing the fixed 50-fixture ownership. The reference validator MUST execute each lifecycle oracle through the public lifecycle transition observer, including authority, base-reference, idempotency, record, effect, and failure-state evaluation; copying table fields into a fixture without observation is insufficient. It MUST validate the Processing Policy, Source Profile, request, scenario, approval receipt, redaction receipt, local attempt-accounting receipt, decision receipt, trusted readback, rule-table, lifecycle, and recovery-report schemas; recompute policy, profile, approval-payload, receipt, request, consent-binding, fixture, and observable-result digests; verify manifest-bound trusted local readback bindings and all 50 oracles; prove every required default-deny code and policy-pair dimension; and validate `conformance/evidence/core-processing-policy-recovery-report.json`. It MUST NOT inspect private helper calls, class shape, or prose that no machine consumes.
