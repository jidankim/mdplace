...
The grants are exhaustive. They MUST explicitly enumerate Intelligence Adapter and provider identifiers, purposes, field and data-class grants, disclosure class, derived artifact kinds, destinations and exact endpoints, credential boundaries, exact consent bindings, per-attempt input/output/runtime/cost budgets, aggregate retry-chain input/output/elapsed-time/cost budgets, an ordered fallback chain, capabilities, non-authoritative semantic scope, and automation scope. Each consent binding correlates one exact adapter, provider, purpose, authorized field and artifact sets, destination, credential boundary, and retention fact; independently enumerated values never create an implicit cross-product. A `local_only` field never authorizes disclosure to an HTTPS destination. Retention facts MUST bind data-use terms, region, and subprocessors. The non-configurable v1 ceiling is one safe-transient retry against the same adapter and one pre-authorized fallback adapter; each fallback row binds its adapter and exact consent binding, and every attempt appears in order in the closed attempt chain. The aggregate retry counters include initial, retry, and fallback attempts. An empty collection grants nothing. A missing field, unknown field, wildcard, duplicate stable identifier, ambient default, inferred provider capability, or unbound fallback grants nothing.
...
| 9 | Retry attempt, aggregate input/output, elapsed time, cumulative cost, same-adapter identity, ordered attempt, or safe-transient outcome violates its exact bound | Terminate the chain and record denial |
