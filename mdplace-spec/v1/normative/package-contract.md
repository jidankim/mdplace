...
The shared requirement, traceability, conformance, and transition meta-schemas MUST accept version-bound downstream area namespaces, scopes, suites, schema paths, transition identifiers, commands, states, categories, and authority roles without revising this foundation contract. The conformance fixture envelope provides a closed `extension` subject whose registered identifier and named downstream schema validate its opaque document and define its observations. Package-release policy remains in the package lifecycle table and semantic checks rather than being encoded as a closed enumeration in the shared meta-schema. The candidate foundation MUST contain all nineteen declared contract schemas and every bound root instance; removing a schema, instance, or binding is a conformance failure.
...
Release evidence MUST include a trusted local-validator receipt from a filesystem observation that enumerates every required slot, records each slot as present, binds the canonical slot-set digest, and proves the expected regular-file or directory type. It MUST also bind artifact verification and both approval receipts to that same manifest digest reference, identify an absent safely creatable immutable target, and publish the specification digest, conformance-pack digest, validator version, independently reproduced validation report, and generated traceability report through manifest-bound references. Amendment evidence MUST identify a safe observation root, complete manifest-bound released-source and draft-target observations, their digests, a strictly greater target version at a different absent path, and exact changed and new requirement identifier sets derived from the source and target catalogs plus their anchored normative sections. An additive-only amendment MAY have an empty changed-identifier set when its new-identifier set is nonempty. The validator MUST reject symbolic links, hard links, non-regular artifacts, wrong slot types, unsafe path components, external targets, occupied release targets, caller-supplied match assertions, literal digest aliases, unattested slot-name lists, and unverified presence claims. Fixture oracles MUST NOT assert private helper calls, internal class shape, or unconsumed prose.
