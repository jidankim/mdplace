...
The shared requirement, traceability, conformance, and transition meta-schemas MUST accept version-bound downstream area namespaces, scopes, suites, schema paths, transition identifiers, commands, states, categories, and authority roles without revising this foundation contract. The conformance fixture envelope provides a closed `extension` subject whose named downstream schema validates its opaque document and whose suite validator defines its observations. Package-release policy remains in the package lifecycle table and semantic checks rather than being encoded as a closed enumeration in the shared meta-schema. The candidate foundation MUST contain all nine declared contract schemas and every bound root instance; removing a schema, instance, or binding is a conformance failure.

The reference validator MUST bound file size, artifact count, schema depth, collection size, reference traversal, and reported errors. A bound violation is a deterministic non-pass result, never an unhandled exception or partial attestation.
