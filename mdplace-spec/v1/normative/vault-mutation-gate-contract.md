...
Resume requires the exact plan, ownership, Descriptor Identity, durable journal prefix, receipt and echo if present, and the next unapplied effect. Exact rollback is permitted only when the plan declares it, every reverse precondition still matches through retained or newly trusted-root-resolved descriptors, and the observed Work Journal contains no later dependent plan; completion additionally requires descriptor readback matching the plan's exact precondition identity. Compensation is permitted only when the plan names the exact compensating plan identity and its separately authorized effect, and completion requires descriptor readback matching that compensating plan's exact expected result identity. Unknown completion, drift, missing evidence, exhausted recovery interruption budget, or unsafe reverse precondition reaches Terminal Manual Repair. Recovery never guesses, duplicates an effect, reports success from console text, or treats a pathname observation as authority.
