...
function scenario(context, index, classes = ['primary']) {
  ...
  return {
    ...
    recovery: {crash_point: 'none', target_attempt_id: null, target_attempt_sequence: null, transmission_observed: false, prior_transmission: null, prior_receipts: []},
...
function definitions(context) {
  return [
    ...
    ['REC', 'crash-before-transmission-recovers-denied', 'crash_recovery', (value) => { value.operation = 'recover'; value.recovery.crash_point = 'before_transmission'; value.recovery.target_attempt_id = value.attempts[0].envelope.attempt_id; value.recovery.target_attempt_sequence = 0; value.attempts[0].double.behavior = 'crash_before_transmit'; value.attempts[0].double.raw_output = null; value.attempts[0].double.provider_request_id = null; }],
    ['REC', 'crash-after-transmission-requires-recovery', 'crash_recovery', (value) => {
      ...
      value.recovery = {crash_point: 'after_transmission_before_receipt', target_attempt_id: value.attempts[0].envelope.attempt_id, target_attempt_sequence: 0, transmission_observed: true, prior_transmission: {destination: value.attempts[0].envelope.destination.endpoint, sha256: sha256(bytes), byte_length: Buffer.byteLength(bytes)}, prior_receipts: []};
    ...
    ['REC', 'crash-after-receipt-preserves-receipt', 'crash_recovery', async (value) => {
      ...
      value.operation = 'recover';
      value.recovery = {crash_point: 'after_receipt', target_attempt_id: value.attempts[0].envelope.attempt_id, target_attempt_sequence: 0, transmission_observed: true, prior_transmission: {destination: priorReceipt.observed_destination, sha256: priorReceipt.transmission_sha256, byte_length: priorReceipt.transmitted_bytes}, prior_receipts: [priorReceipt]};
    }],
  ];
}
...
async function generateFixturesAndEvidence() {
  ...
  const recoveryReport = {
    ...
    cases: recoveryRecords.map(({fixtureId, document, expected, receiptDigests}) => ({
      ...
      target_attempt_sequence: document.recovery.target_attempt_sequence,
